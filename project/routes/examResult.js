...
//     examResponses.forEach((document) => {
//       let totalMarks = 0;
//       let obtainedMarks = 0;
...
//             obtainedMarks += question.marks; // Correct answer
...
const validateAnswer = async (question, userAnswer) => {
  ...
  } else if (question.type === 'short_answer' || question.type === 'long_answer' || question.type == 'short' || question.type == 'long') {
    try {
      ...
      return response; // Adjust this based on actual API response
    } catch (error) {
    ...
    for (const document of examResponses) {
      ...
      for (const response of document.responses) {
        ...
        if (question) {
          ...
          const isCorrect = await validateAnswer(question, response.selectedOption);
